...
def CalculateLR(p, N, chaneloutput_y, i, estimatedcodeword_u, LRmatrix, branch):
    """
    尤度比LRを計算する
    N:符号長
    ...
    if i % 2 == 0:
        LR1 = CalculateLR(p, int(N/2), y_1, int(j/2),
                          hat_u1, LRmatrix, 2*branch)
        LR2 = CalculateLR(p, int(N/2), y_2, int(j/2),
...
if __name__ == "__main__":
    ...
    a = CalculateLR(P, 1, np.array([0]), 0, np.array([]), TE, 0)
    print(type(a))